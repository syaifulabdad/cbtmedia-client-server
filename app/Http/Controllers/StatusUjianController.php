...
class StatusUjianController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('status-ujian')->with([
            ...
            'ujian' => $ujian,
            // 'getJadwal' => Jadwal::orderBy('hari_ke', 'asc')->orderBy('tanggal', 'asc')->whereRelation('bankSoal', 'status', 1)->get(),
            'getJadwal' => Jadwal::whereRelation('bankSoal', 'status', 1)
                ->groupBy('hari_ke', 'tanggal')
                ->orderBy('hari_ke', 'asc')
                ->orderBy('tanggal', 'asc')
                ->get([
                    'hari_ke',
                    'tanggal',
                    \DB::raw('MIN(id) as id') // Ambil ID terkecil dari setiap grup unik
                ]),
    ...
    public function dataTables(Request $request)
    {
        ...
        $datatables->addColumn('nama_mapel', function ($row) {
            $dt = "<span class='fw-bold text-uppercase'>" . $row->bankSoal->mataPelajaran->nama . "</span>";
        ...
        $datatables->addColumn('tingkat', function ($row) {
            return $row->bankSoal->tingkat;
        });
        $datatables->addColumn('nama_jurusan', function ($row) {
            return $row->bankSoal->jurusan ? $row->bankSoal->jurusan->nama : 'Semua';
        });
        $datatables->addColumn('kelas', function ($row) {
            $dt = '<table class="table-dt">';
            $dt .= "<tr><td>Tingkat </td><td> : <b>" . $row->bankSoal->tingkat . '</b></td></tr>';
        ...
        $datatables->addColumn('jumlah_peserta', function ($row) {
            $dt = '<table class="table-dt">';
            $dt .= "<tr><td>Semua </td><td> : <a href='javascript:void(0)'><b>" . ($row->statusPesertaUjian ? $row->statusPesertaUjian->count() : null) . "</b></a></td></tr>";
            $dt .= "<tr><td>Aktif </td><td> : <a href='javascript:void(0)'><b>" . ($row->statusPesertaUjian ? $row->statusPesertaUjian->where('status', 1)->count() : null) . "</b></a></td></tr>";
            $dt .= "<tr><td>Selesai </td><td> : <a href='javascript:void(0)'><b>" . ($row->statusPesertaUjian ? $row->statusPesertaUjian->where('status', 0)->count() : null) . "</b></a></td></tr>";
            $dt .= '</table>';
            return $dt;
        });
        ...
        $datatables->addColumn('status', function ($row) {
            if ($row->status == 1) {
                return '<a href="javascript:void(0)" class="btn btn-info mt-1 mb-1 btnStatus btnStatus_' . $row->id . '" data-id="' . $row->id . '">AKTIF</a><script>startCountdownSoal("' . $row->waktu_ujian . '", "' . $row->alokasi_waktu_soal . '", "' . date('Y-m-d H:i:s') . '", "' . $row->id . '");</script>';
            } else {
                return '<a href="javascript:void(0)" class="btn btn-warning mt-1 mb-1 btnStatus btnStatus_' . $row->id . ' btnSelesai" data-id="' . $row->id . '">SELESAI</a>';
            }
        });
        ...
        $datatables->addColumn('action', function ($row) {
            ...
            if ($row->statusPesertaUjian->count() == 0)
    ...
    {
        $getData = Model::with(['bankSoal', 'jadwal'])->find($id);
        ...
        $getData->nama_mapel = $bankSoal->mataPelajaran ? $bankSoal->mataPelajaran->nama : null;
    ...
    public function postStore(Request $request)
    {
        ...
        $data['nama_mapel'] = $bankSoal->mataPelajaran ? $bankSoal->mataPelajaran->nama : null;
    ...
    {
        $bankSoal = BankSoal::whereRelation('jadwal', 'hari_ke', $request->hari_ke)
            ->whereRelation('jadwal', 'tanggal', $request->tanggal)
            ->where('status', 1);

        $result = null;
        foreach ($bankSoal->get() as $soal) {
            $result .= "<option value='$soal->id'>" . $soal->mataPelajaran->nama . "</option>";
        }
    ...
    {
        $getData = BankSoal::with(['jadwal', 'mataPelajaran', 'jurusan'])
            ->whereRelation('jadwal', 'hari_ke', $request->hari_ke)
            ->whereRelation('jadwal', 'tanggal', $request->tanggal)
            ->find($request->bank_soal_id);
        ...
        $getData->nama_mapel = $getData->mataPelajaran ? $getData->mataPelajaran->nama : null;
        return response()->json($getData);
    }
}
