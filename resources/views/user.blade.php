...
@section('content')
                ...
                <div class="card">
                    <div class="card-header  border-0 d-flex align-items-center">
                        <div class="col">
                            ...
                            @if ($user_type == 'ptk')
                                <a href="javascript:void(0)" class="btn btn-info btnGeneratePTK ms-3"><i class="ri-add-line me-1"></i> Buat Akun PTK</a>
                            @endif

                            @if ($user_type == 'siswa')
                                <a href="javascript:void(0)" class="btn btn-warning btnGenerateSiswa ms-3"><i class="ri-add-line me-1"></i> Buat Akun Siswa</a>
                            @endif
                        </div>
                        <div class="col-auto">
                    ...
                    <div class="card-header border-0 d-flex align-items-center pt-0 pb-0">
                        <div class="row {{ $user_type == 'ptk' ? 'd-none' : null }}">
                            <div class="col">
                                <select id="ujian_id" class="form-control form-select" onchange="reload()">
                                    <option value="">Data Ujian</option>
                                    @foreach ($getUjian as $ujian)
                                        <option value="{{ $ujian->id }}" {{ $ujian->status ? 'selected' : null }}>{{ $ujian->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
    ...
    <div class="modal fade" id="modalGenerateSiswa" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formGenerateSiswa" class="tablelist-form" autocomplete="off">
                        @csrf
                        <div class="row mb-2">
                            <label for="" class="col-sm-12 col-form-label">Data Ujian</label>
                            <div class="col-md-12">
                                <select name="ujian_id" class="form-control form-select">
                                    @foreach ($getUjian as $ujian)
                                        <option value="{{ $ujian->id }}" {{ $ujian->status ? 'selected' : null }}>{{ $ujian->nama }}</option>
                                    @endforeach
                                </select>
                ...
                <div class="modal-footer">
                    <div class="hstack gap-2 justify-content-end">
                        <button type="button" class="btn btn-light" data-bs-dismiss="modal">Close</button>
                        <a href="javascript:void(0)" class="btn btn-success btnGenerateSiswaSave"><i class="bx bx-save"></i> Simpan</a>
                    </div>
                </div>
            </div>
        </div>
...
@section('script')
    ...
    <script>
        ...
        @if ($user_type == 'ptk')
            $('.btnGeneratePTK').click(function() {
                $('.btnGeneratePTK').attr('disabled', true).html('menyimpan...');
                $.ajax({
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
                    url: "{{ $cUrl }}/generate-ptk",
                    dataType: 'json',
                    type: "POST",
                    success: function(data) {
                        if (data.status) {
                            ...
                            Swal.fire({
                                title: "Sukses.!",
                                text: 'Akun PTK berhasil dibuat.',
                                icon: "success",
                            });
                        } else {
                            alert('Data gagal disimpan.!!');
                        }
                        $('.btnGeneratePTK').attr('disabled', false).html('<i class="ri-add-line me-1"></i> Buat Akun PTK');
                    },
                    error: function(jqXHR, textStatus, errorThrown) {
                        alert(errorThrown);
                        $('.btnGeneratePTK').attr('disabled', false).html('<i class="ri-add-line me-1"></i> Buat Akun PTK');
        ...
        @if ($user_type == 'siswa')
            $('.btnGenerateSiswa').click(function() {
                $('[name="ujian_id"]').val($('#ujian_id').val());
                $('.modal-title').html('Buat Akun Siswa');
                $('#modalGenerateSiswa').modal('show');
            });

            $('.btnGenerateSiswaSave').click(function() {
                $('.btnGenerateSiswaSave').attr('disabled', true).html('menyimpan...');

                var formData = $('#formGenerateSiswa').serialize();
                $.ajax({
                    data: formData,
                    url: "{{ $cUrl }}/generate-siswa",
                    type: "POST",
                    dataType: 'json',
                    success: function(data) {
                        if (data.status) {
                        ...
                        $('.btnGenerateSiswaSave').attr('disabled', false).html('<i class="bx bx-save"></i> Simpan');
                    },
                    error: function(jqXHR, textStatus, errorThrown) {
                        alert(errorThrown);
                        $('.btnGenerateSiswaSave').attr('disabled', false).html('<i class="bx bx-save"></i> Simpan');
